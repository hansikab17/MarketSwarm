    ...
    <div className="overlay" onClick={onCancel}>
      <div className="modal cfg-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="cfg-body" id="cfgBody">
          ...
          <Slider label="Human respondents" icon="👥" value={config.humanPct} min={0} max={100} step={5}
          ...
          <Slider label="Number of customers" icon="👥" value={config.count} min={50} max={50000} step={50}
          ...
          <Slider label="Gender split" icon="🎭" value={config.genderSplit} min={0} max={100} step={1}
            fmt={(v) => `${v}% F / ${100-v}% M`} onChange={(v) => set("genderSplit", v)} />
          <div className="row">
            <div>
              <Slider label="Youngest age" icon="🎂" value={config.ageMin} min={18} max={config.ageMax} step={1}
                fmt={(v) => String(v)} onChange={(v) => set("ageMin", v)} />
            </div>
            <div>
              <Slider label="Oldest age" icon="🎂" value={config.ageMax} min={config.ageMin} max={80} step={1}
          ...
          <Slider label="Average customer budget" icon="💰" value={config.budget} min={10} max={500} step={5}
            fmt={(v) => `$${v}`} onChange={(v) => set("budget", v)} />
          <Slider label="New product price" icon="🎯" value={config.price} min={5} max={500} step={1}
            fmt={(v) => `$${v}`}
          ...
          <Slider label="Product–market fit" icon="🎯" value={config.fit} min={0} max={100} step={1}
            fmt={(v) => `${v}/100`} hint="How well the product matches what this cohort wants" onChange={(v) => set("fit", v)} />
          <Slider label="Marketing push" icon="📈" value={config.marketing} min={0} max={100} step={1}
            fmt={(v) => `${v}/100`} hint="Reach and intensity of the go-to-market campaign" onChange={(v) => set("marketing", v)} />
          <Slider label="Innovation / novelty" icon="✨" value={config.innovation} min={0} max={100} step={1}
            fmt={(v) => `${v}/100`} hint="Rewards early-adopter cohorts, can deter the risk-averse" onChange={(v) => set("innovation", v)} />
          <Slider label="Brand strength" icon="⭐" value={config.brandFit} min={0} max={100} step={1}
